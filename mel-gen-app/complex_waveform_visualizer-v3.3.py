"""
Complex Waveform Visualizer  v3.3
===================================
...
Changes in v3.3
  - Figure height increased to 12" so Voice 4 panel is no longer clipped.
  - Voice On/Show/Hz checkboxes split into three side-by-side widgets
    (matplotlib's CheckButtons stacks labels vertically; in a short axes
    they overlap).
  - Clipper mode replaced with a custom horizontal radio so the four
    mode labels (hard/soft/fold/wrap) sit on one row instead of stacking.
  - Larger gap between the Velocity slider and the Pause/Reset buttons.
...
Usage:   python3 complex_waveform_visualizer-v3.3.py

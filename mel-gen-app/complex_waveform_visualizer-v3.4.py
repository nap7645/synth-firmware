...
Changes in v3.4
  ...
  - Per-voice saturation rewritten as a fixed soft-knee saturator
    (linear below 0.8*amp, tanh rolloff approaching ±amp). Now fully
    decoupled from the global clipper mode, so hard/fold/wrap apply
    only to the summed signal where they belong.
  - Internal/External clock selector added in the GLOBAL section;
    the engine no longer auto-overrides the BPM slider when external
    clock is detected unless the user has selected External.
  - Clock receiver now opens every available MIDI input port so DAWs
    routing clock to an IAC bus (or any existing port) reach us
    automatically — no need to know our virtual port's name.
  - Real audio output engine (sounddevice). Visible only in 'audio'
    mode: enable + master volume. Voice freq in audio mode is
       Hz mode    : voice's Hz slider value
       ratio mode : ratio * master Freq slider
    so ratios become harmonic multipliers of the fundamental.
  - Status line shows both MIDI and audio engine state.
...
Usage:   python3 complex_waveform_visualizer-v3.4.py
Requires: numpy, matplotlib, python-rtmidi, sounddevice (optional, for audio out)
"""
...
try:
    ...
    # OSError can be raised on systems without PortAudio installed
...
# ===============================================================================
#  Audio Engine — real-time synthesis to system audio output
# ===============================================================================

class AudioEngine:
    """Real-time synthesis driver.

    sounddevice runs a background audio thread that calls _audio_cb every
    block_size samples. The callback generates each voice's contribution
    to the current block using persistent phase accumulators (so block
    boundaries stay phase-continuous), sums them, applies the same per-
    voice soft-knee saturator and global clipper used by the visualizer,
    multiplies by master volume, and writes the result into the output
    buffer.

    Voice frequency in audio mode:
      - Hz mode  : voice runs at its own Hz slider value
      - Ratio mode: voice runs at  ratio * audio_freq_slider  Hz
        (so the master Freq slider acts as the fundamental; the ratios
        become harmonic multipliers, which gives clock-mode-style ratio
        relationships an audible interpretation.)

    Audio output is meaningful in 'audio' mode only. Switching the mode
    to 'clock' auto-stops the stream so we never blast a sub-audio-rate
    contour as DC out the speaker.
    """

    SAMPLE_RATE = 44100
    BLOCK_SIZE  = 512
    FADE_MS     = 5         # fade in/out to avoid clicks on start/stop
    ...
    def __init__(self):
        ...
        self._fade_state     = 0.0   # 0 = silent, 1 = full volume
        self._fade_target    = 0.0
        self._fade_per_sample = 1.0 / (self.SAMPLE_RATE * self.FADE_MS * 1e-3)

    def _shape_block(self, shape_idx, phase_start, phase_step, n):
        """Generate `n` samples of a waveform starting at phase_start."""
        phases = phase_start + phase_step * np.arange(n)
        t = phases % 1.0
        if shape_idx == 0:    # sine
            return np.sin(2 * np.pi * t)
            ...
            return np.where(t < 0.5, 1.0, -1.0)
        elif shape_idx == 3:  # ramp up
            return 2.0 * t - 1.0
        elif shape_idx == 4:  # ramp dn
            return 1.0 - 2.0 * t
        return np.zeros(n)
    ...
    def _audio_cb(self, outdata, frames, time_info, status):
        try:
            sr      = self.SAMPLE_RATE
            master  = float(sl_audio_vol.val) if 'sl_audio_vol' in globals() else 0.5
            audio_f = float(sl_audiofreq.val)
            ...
            for i in range(4):
                ...
                if not st.on:
                    continue
                amp_val = float(vs['amp'].val)
                if amp_val <= 1e-6:
                    continue
                shape_idx = int(vs['shape'].val)
                phase_deg = float(vs['phase'].val)

                # Voice frequency selection
                if voice_hz_modes[i]:
                    f_hz = 10.0 ** float(vs['hz'].val)
                else:
                    ratio = RATIO_VALUES[int(vs['ratio'].val)]
                    f_hz  = ratio * audio_f

                if f_hz <= 0 or f_hz > sr * 0.45:
                    # Skip silent / above-Nyquist voices
                    continue

                phase_step = f_hz / sr
                phase_off  = phase_deg / 360.0
                wf = self._shape_block(
                    shape_idx, self._phases[i] + phase_off, phase_step, frames
                )
                wf = amp_val * wf
                wf = soft_knee_saturate(wf, amp_val, knee=0.8)
                block += wf

                # Advance phase for next callback
                self._phases[i] = (self._phases[i] + phase_step * frames) % 1.0

            # Global stage: G.Amp scale, DC shift, global clipper
            block = float(sl_gamp.val) * block + float(sl_gdc.val)
            block = apply_clip(block, float(sl_clip_thresh.val), clip_mode)

            # Soft-fade for click-free start/stop
            if self._fade_state != self._fade_target:
                fade = np.linspace(
                    self._fade_state,
                    self._fade_state + self._fade_per_sample * frames,
                    frames,
                    dtype=np.float64,
                )
                fade = np.clip(fade, 0.0, 1.0)
                self._fade_state = float(fade[-1])
                if self._fade_target == 0.0:
                    fade = 1.0 - fade
                    self._fade_state = float(fade[-1])
                block = block * fade
            else:
                block = block * self._fade_state

            # Final: scale by master volume, then divide by 4 to keep
            # peaks roughly in [-1, 1] for typical 4-voice sums
            out = (master * 0.25) * block
            np.clip(out, -1.0, 1.0, out=out)
            outdata[:, 0] = out.astype(np.float32)
        except Exception as e:
            self.status = f'Audio cb error: {e}'
            outdata.fill(0)
    ...
    def start(self):
        ...
        try:
            ...
            self._stream.start()
            self.active = True
            self.status = f'Audio out @ {self.SAMPLE_RATE} Hz'
        except Exception as e:
    ...
    def stop(self):
        if not self.active or self._stream is None:
            ...
            self.status = 'Audio off'
            return
        # Schedule fade-out, then close after one fade duration
        self._fade_target = 0.0
        try:
            time.sleep(self.FADE_MS * 1e-3 * 2)
            self._stream.stop()
            self._stream.close()
        except Exception:
            pass
...
# Mode (clock vs audio) and Clock source (Int vs Ext) radios on one row
y -= 0.045
mode_w = L_WIDTH * 0.48
clock_src_w = L_WIDTH * 0.48
ax_mode = fig.add_axes([L_MARGIN, y, mode_w, 0.035], facecolor=BG_DARK)
radio_mode = RadioButtons(ax_mode, ['clock', 'audio'], active=0,
                          activecolor='#3498db')
style_check(radio_mode)

ax_clock_src = fig.add_axes(
    [L_MARGIN + L_WIDTH - clock_src_w, y, clock_src_w, 0.035],
    facecolor=BG_DARK,
)
...
# Audio output enable + master volume — visible only in audio mode.
# Placed in their own absolute slot so toggling mode doesn't reshuffle
# the layout above. When clock mode is active, both axes are hidden.
y -= SL_H + GAP + SECT_GAP
ax_audio_on = fig.add_axes([L_MARGIN, y, L_WIDTH * 0.5, CHK_H],
                           facecolor=BG_DARK)
chk_audio = CheckButtons(ax_audio_on, ['Audio Out'], [False])
style_check(chk_audio, '#3498db')
ax_audio_on.set_visible(False)

y -= CHK_H + GAP
ax_audio_vol, sl_audio_vol = make_slider(y, 'Vol', 0.0, 1.0, 0.4, 0.01,
                                         '#3498db')
ax_audio_vol.set_visible(False)
...
def _refresh_midi_status():
    # Combine MIDI and audio engine status into the single status line.
    parts = []
    if midi_engine.active:
        parts.append(midi_engine.status)
    if audio_engine.active:
        parts.append(audio_engine.status)
    if not parts:
        # Show whichever engine has a more interesting message (e.g. error)
        if 'error' in audio_engine.status.lower() or 'not installed' in audio_engine.status.lower():
            parts.append(audio_engine.status)
        elif 'error' in midi_engine.status.lower() or 'not installed' in midi_engine.status.lower():
            parts.append(midi_engine.status)
        else:
            parts.append('MIDI off')
    color = MIDI_COLOR if (midi_engine.active or audio_engine.active) else '#888888'
    midi_status_text.set_text(' | '.join(parts))
...
def on_mode_change(label):
    ...
    # Audio output controls only appear in audio mode. If we're leaving
    # audio mode while the stream is running, stop it cleanly.
    ax_audio_on.set_visible(not is_clock)
    ax_audio_vol.set_visible(not is_clock)
    if is_clock and audio_engine.active:
        audio_engine.stop()
        # Reflect the auto-stop in the checkbox visual
        try:
            if chk_audio.get_status()[0]:
                chk_audio.set_active(0)
        except Exception:
            pass
    update()
...
# Audio out enable/disable
def on_audio_toggle(label):
    if audio_engine.active:
        audio_engine.stop()
    else:
        audio_engine.start()
    _refresh_midi_status()
chk_audio.on_clicked(on_audio_toggle)

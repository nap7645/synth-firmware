"""
Complex Waveform Visualizer  v3.2
===================================
...
Changes in v3.2
  ...
  - Transport button layout fixed (no overlap with Velocity).
...
Usage:   python3 complex_waveform_visualizer-v3.2.py
...
def build_voct_array(complex_s):
    ...
    for i in range(4):
        ...
        phase   = vs['phase'].val
        if voice_hz_modes[i]:
            wf = generate_voice_hz(shape, vs['hz'].val, amp_val, phase, t)
        else:
            wf = generate_voice(shape, RATIO_VALUES[int(vs['ratio'].val)],
                                amp_val, phase, complex_s, t)
        # Per-voice saturation at 80% of amp knob
        thresh = 0.8 * amp_val
        if thresh > 1e-6:
            wf = apply_clip(wf, thresh, clip_mode)
        active_wf.append(wf)
...
y -= 0.038
ax_clip_mode = fig.add_axes([L_MARGIN, y, L_WIDTH, 0.034], facecolor=BG_DARK)
radio_clip = RadioButtons(ax_clip_mode, CLIP_MODES, active=1,
                          activecolor=CLIP_COLOR)
style_check(radio_clip, CLIP_COLOR)
...
# Transport: Play/Pause  Reset  — extra gap to avoid overlapping velocity
y -= SECT_GAP + 0.018
btn_w = (L_WIDTH - GAP) / 2
...
for v in range(4):
    ...
    y -= CHK_H + GAP
    ax_chk = fig.add_axes([L_MARGIN, y, L_WIDTH * 0.6, CHK_H], facecolor=BG_DARK)
    vs['chk'] = CheckButtons(ax_chk, ['On', 'Show', 'Hz'], [True, True, False])
    ...
    # Hz slider (free Hz mode) — same y position, hidden by default
    ax_hz = fig.add_axes([L_MARGIN, y, L_WIDTH, SL_H], facecolor=BG_CTRL)
    sl_hz = Slider(ax_hz, 'Hz', 0.1, 2000.0, valinit=VOICE_HZ_DEFAULT,
                   valstep=0.1, color=color, initcolor='none')
    style_slider(sl_hz)
    slider_defaults[sl_hz] = VOICE_HZ_DEFAULT
    ...
    for i in range(4):
        ...
        if st.on:
            ...
            if voice_hz_modes[i]:
                ...
                wf = generate_voice(shape, RATIO_VALUES[int(vs['ratio'].val)],
                                    amp_val, phase, complex_s, t)
            # Per-voice saturation at 80% of amp
            thresh = 0.8 * amp_val
            if thresh > 1e-6:
                wf = apply_clip(wf, thresh, clip_mode)
            active_wf.append(wf)
...
# Clipper
def on_clip_mode_changed(label):
    global clip_mode
    clip_mode = label
    midi_engine.mark_dirty()
    update()
radio_clip.on_clicked(on_clip_mode_changed)
...
def make_voice_checkbox_cb(idx):
    def cb(label):
        ...
        if label == 'On':
            ...
            # Toggle visibility of ratio vs hz slider

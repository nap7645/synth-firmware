...
SCALES = {
    ...
    # Harmonics 1-16 mapped to nearest 12-TET semitone, reduced to one octave,
    # deduplicated: root, maj2, maj3, P5, min7, maj9, maj10, P12
...
class MidiEngine:
    ...
    def _compute_sequence(self):
        """Compute the full quantized waveform and return a list of
        (midi_note, duration_seconds) pairs by detecting value transitions.

        Always starts from t=0 so the same knob state always produces the
        same melody (fully deterministic). Recomputed fresh every loop.
        """
        try:
            bpm      = sl_bpm.val
            ...
            seconds_per_sample = complex_s / SAMPLES

            # Build combined waveform from t=0 (deterministic)
            active_wf = []
            for i in range(4):
                vs = voice_sliders[i]
                st = voice_states[i]
            ...
            # Convert entire V/oct array to MIDI notes
            midi_seq = np.array([self.voct_to_midi(v) for v in voct_seq])

            # Find transition indices: positions where the note value changes
            transitions = np.concatenate(([0], np.where(np.diff(midi_seq) != 0)[0] + 1))

            # Build (note, duration_s) pairs from run-length encoding
            events = []
            for k, start_idx in enumerate(transitions):
                end_idx = transitions[k + 1] if k + 1 < len(transitions) else SAMPLES
                note_num = int(midi_seq[start_idx])
                duration = (end_idx - start_idx) * seconds_per_sample
                events.append((note_num, duration))
    ...
    def _run(self):
        while not self._stop_evt.is_set():
            # Recompute fresh from t=0 at the top of every loop
            events, bpm = self._compute_sequence()
            ...
            for note, duration in events:
                ...
                gate_dur    = duration * gate_frac
                rest_dur    = duration - gate_dur

                # Legato: send note-on before note-off of previous note
                # so there is no gap between adjacent notes
                self._note_on(channel, note, velocity)
                if self._last_note is not None and self._last_note != note:
                    self._note_off(channel, self._last_note)
                    self._last_note = note

                time.sleep(gate_dur)

                # Only send note-off mid-note when gate < 1.0
                if rest_dur > 0:

...
/* Clock divider counter. Deliberately independent of seq.step: if it were
   derived from the pattern position, an N that isn't a multiple of
   SEQ_CLOCK_DIV would produce a short gap at every cycle wrap. Reset on
   start/stop so the clock stays phase-locked to the downbeat. */
static volatile uint8_t clk_ctr;
...
void seq_on_step(void)
{
    ...
    /* SOC marks the downbeat; clock out fires every SEQ_CLOCK_DIV steps. Both
       are software driven -- PC4 has no compare channel at all (TIM1_ETR only)
       and PA7's remaining option collided with the N encoder's timer. */
    uint8_t fired = 0U;

    if (s == 0U)
    {
        SOC_PORT->BSRR = SOC_PIN;
        fired = 1U;
    }
    ...
    if (++clk_ctr >= (uint8_t)SEQ_CLOCK_DIV) clk_ctr = 0U;

    /* Arm the one-shot that will drop them again -- only if something fired,
       so an idle step costs nothing. */
    if (fired)
    {
